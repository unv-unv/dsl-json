...
import com.dslplatform.json.runtime.BeanNaming;
...
public class Analysis {
	...
	public Map<String, StructInfo> analyze() {
		...
		for (Map.Entry<String, StructInfo> it : structs.entrySet()) {
			...
			if (!info.hasKnownConversion() && info.usesBuilder()) {
	...
	public static String beanOrActualName(String name, boolean isBoolean) {
		return BeanNaming.beanOrActualName(name, isBoolean);
	}
	...
	private static boolean isNonNullByJSpecify(Element property, @Nullable VariableElement field) {
		...
		return !isMarkedNullable(property) && (field == null || !isMarkedNullable(field));
	}
	...
	private static final Set<String> NULLABLE_ANNOTATIONS = new HashSet<String>(Arrays.asList(
	...
	private static boolean isMarkedNullable(Element element) {
	...
	private static boolean containsNullable(List<? extends AnnotationMirror> annotations) {
		for (AnnotationMirror ann : annotations) {
			if (NULLABLE_ANNOTATIONS.contains(ann.getAnnotationType().toString())) return true;
		}
		return false;
	}
	...
	private @Nullable NamingStrategy namingStrategy(TypeElement element, @Nullable AnnotationMirror annotation) {
		...
		if (strategyName == null
				|| DefaultNamingStrategy.class.getName().equals(strategyName)
				|| NamingStrategy.class.getName().equals(strategyName)) return null;
