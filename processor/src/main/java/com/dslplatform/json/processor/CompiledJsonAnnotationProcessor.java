	...
	public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
		...
		if (!compiledJsons.isEmpty() || !jacksonCreators.isEmpty() || !jsonbCreators.isEmpty()) {
			...
			for (Map.Entry<String, StructInfo> entry : structs.entrySet()) {
				...
				if (!roundEnv.getRootElements().contains(structInfo.element)
						&& processingEnv.getElementUtils().getTypeElement(classNamePath) != null) {
					//converter was already generated by the compilation which owns this type.
					//regenerating it here would duplicate the class into a package owned by another module
					continue;
				}
				try {
